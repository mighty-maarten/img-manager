...
# ==========================================
# System-wide NODE_ENV Configuration Module
# ==========================================
log "Starting system-wide NODE_ENV configuration..."
...
log "✓ System-wide NODE_ENV configuration module completed"

# ==========================================
# Secrets Configuration Module
# ==========================================
log "Starting secrets configuration for deployment scripts..."

# Create secrets-config.sh file for deployment scripts to use
# This file will be sourced by merge-env.sh during CodeDeploy deployments
log "Creating secrets configuration file..."
cat > /opt/img-manager/shared/secrets-config.sh << EOF
# Secrets configuration for deployment scripts
# This file is sourced by merge-env.sh to retrieve database credentials
# Generated by EC2 init script at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")
...
log "✓ Secrets configuration file created at /opt/img-manager/shared/secrets-config.sh"

# Set file permissions to 600 for security (contains secret ARN)
log "Setting secrets-config.sh file permissions..."
chown ec2-user:ec2-user /opt/img-manager/shared/secrets-config.sh
chmod 600 /opt/img-manager/shared/secrets-config.sh

log "✓ secrets-config.sh file permissions set (600)"

# Verify secrets-config.sh file exists with correct permissions
log "Verifying secrets-config.sh file..."
if [ -f "/opt/img-manager/shared/secrets-config.sh" ]; then
    PERMS=$(stat -c "%a" /opt/img-manager/shared/secrets-config.sh)
    if [ "$PERMS" = "600" ]; then
        log "✓ secrets-config.sh file exists with correct permissions (600)"
    else
        log "⚠ Warning: secrets-config.sh file has incorrect permissions ($PERMS, expected 600)"
    fi
else
    log "✗ Error: secrets-config.sh file not found"
    exit 1
fi

log "✓ Secrets configuration module completed"

# NOTE: .env.production is NO LONGER created here.
# It will be generated during CodeDeploy deployment by merge-env.sh,
# which merges build-time config (.env.build) with runtime secrets from Secrets Manager.
...
Configuration Files:
-------------------
✓ /opt/img-manager/shared/secrets-config.sh (secrets ARN for deployments)
...
Note: .env.production is generated during CodeDeploy deployment (not EC2 init)
